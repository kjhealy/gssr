...
The panel data objects are of class `panel_data`, a type of tibble created by `panelr`. The column names in long format do not have wave identifiers. Rather,  `firstid` and `wave` variables track the cases:

```{r panel-example}
gss_panel06_long %>% select(sex, race)
```
